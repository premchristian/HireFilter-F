...
const navItems = [
    ...
    { name: "Company Profile", href: "/hr/profile", icon: Building2 },
    { name: "Post a Job", href: "/hr/jobs/create", icon: PlusCircle },
    { name: "All Jobs", href: "/hr/jobs", icon: Briefcase },
    // { name: "Resume Analyzer", href: "/hr/resume-analyzer", icon: FileSearch },
    // { name: "Create Exam", href: "/hr/exams/create", icon: FileEdit },
    // { name: "All Exams", href: "/hr/exams", icon: FileText },
    { name: "Applications", href: "/hr/applicants", icon: Users },
    { name: "Shortlisted", href: "/hr/shortlist", icon: UserCheck },
             ...
             try {
                const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/users/getProfile`, {
                 ...
                 if (res.data?.success) {
                    ...
                    const profileLogo = getUrl(data?.company?.logo) || getUrl(data?.profileImage) || getUrl(data?.profile?.image);
                    setUserImage(profileLogo);
                    
                     if (data?.company?.name || data?.name) {
                         setUserName(data?.company?.name || data?.name);
                     }
                }
             } catch (e) {
                ...
                <Link href="/hr/profile">
                    <div className="mt-4 flex items-center gap-3 px-4 bg-[#F4F7FE] p-3 rounded-[12px] border border-[#F1F1F1] shadow-[0px_4px_20px_rgba(0,0,0,0.05)] hover:border-[#7C5CFC]/20 hover:bg-[#EBE8FF]/50 transition-all cursor-pointer group/profile">
                        ...
                        <div className="flex flex-col overflow-hidden">
                            ...
                            <span className="text-xs text-[#71717A] font-medium truncate">HR Manager</span>
                        </div>
                    </div>
                </Link>
            </div>
    ...
    return (
        <div className="min-h-screen bg-[#F4F7FE] flex font-sans text-[#080808] overflow-hidden transition-colors duration-300">
                ...
                <main className="flex-1 relative z-10 lg:overflow-y-auto h-screen scrollbar-hide flex flex-col">
                    ...
                    {/* Main Scroll Area */}
                    <div className="flex-1 overflow-y-auto scrollbar-hide p-4 lg:p-8 pt-0">
